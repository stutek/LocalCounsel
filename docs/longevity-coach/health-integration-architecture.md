...
* **Local Anonymization Filter (PII Scrubbing)**: Before any prompt leaves the local machine, a dedicated tool in the Dify workflow scrubs all Personally Identifiable Information (PII) including:
  ...
  * Exact Age (fuzzed dynamically by introducing a random +/- 10% variance offset to prevent database cross-matching re-identification). *Implemented for the BIA nutrition path in `health_sync.nutrition.SubjectProfile.fuzzed_age` / `build_nutrition_prompt`: sex is included and the age is emitted only as a single randomized value within ±10%, never the exact age.* The ±10% band (rather than a wider one) is a **deliberate privacy/advice-quality trade-off**: a tighter age estimate lets the model give more age-appropriate guidance while still never revealing the exact age. Do not widen it without a reason.
